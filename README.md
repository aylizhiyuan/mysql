...
这里,数据段里面放的就是我们的页---注意看我们左边的大小,页会根据区进一步的划分,一个区里会存放16,384个页,一共是256M大小的页,每到达16,384个页，就需要一个XDES来分割确保这是一个区

这里面第一个页FSP_HDR维护了这个区的相关信息,跟XDES一样，也是为了记录该区的一些详细信息的
...
我们再来看这张图，这是一个典型的表中页的结构,这里面需要关注的是
INODE和INDEX

![](./images/index6.png)

个人感觉更像是将这些页面以b+树的形式存储起来,从root page(树的根节点)开始,根据inodes节点的信息找到区内具体的pages
